...
Gaze-only encoder–decoder transformer. Takes the noisy gaze trajectory (`src`) and autoregressively predicts clean fixations (`tgt`). No image features. Used as an ablation baseline and for fast iteration. Input encoders: `linear` or `shared_gaussian`. **Trainable end-to-end through the pipeline with scheduled sampling** (spec `spec/2026-09-08-revive-path-model-training/`): it carries the same four scheduled-sampling seams as `MixerModel` — `phase`/`scheduled_sampling` attrs, `set_scheduled_sampling` (the method `pipeline.py:62` requires), a phase-recording `set_phase` (no `requires_grad` toggling — single param group), a no-op `decode_denoise` → `{}`, and a sampler-aware `forward(skip_denoise=False, **kwargs)`. The `ScheduledSampling.__call__` contract is met unchanged (no `enable_memory_kv_cache`, so its `hasattr` guard skips it; `decode_denoise` only reached on a `Combined` phase PathModel never enters). Runs **Fixation-only** with `separated_reg` loss + `multi_mlp` head (matching `MixerModel`'s fixation objective, so the ablation isolates *image features*), via `configs/exp/path_model_training.yaml`. `training.reuse_split_from` (a model-agnostic `build_model` key) lets it reuse a reference `MixerModel` run's `split.pth` for an apples-to-apples comparison.
...
| `PathModel` | `src/model/path_model.py` | Gaze-only baseline. Simpler: single encoder stack → decoder stack → head. Now carries the scheduled-sampling seams (`set_scheduled_sampling`, `phase`, no-op `decode_denoise`, sampler-aware `forward`) so it trains end-to-end and eval-time autoregression runs through the sampler; `set_phase()` records `self.phase` (no `requires_grad` toggling — one param group). Trained Fixation-only via `configs/exp/path_model_training.yaml`; can reuse a MixerModel split via `training.reuse_split_from`. |
