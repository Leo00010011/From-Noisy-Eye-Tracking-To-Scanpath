#!/bin/bash
#SBATCH --job-name=train_ia
#SBATCH --output=logs/ia_out_%j.log
#SBATCH --error=logs/ia_err_%j.log
...
echo "STARTING IMAGE-ADAPTATION TRAINING (precomputed frozen Mask2Former features, img_size=512)"
# 3-phase run: ImageAdaptation -> Combined (adapter frozen) -> FullFinetune (pixel decoder frozen).
# The exp config already sets model/image_encoder=mask2former_precomputed,
# data.load.use_precomputed_features=True, model.image_adaptation.enabled=True, and the phases.
...
python train.py exp=image_adaptation_training \
